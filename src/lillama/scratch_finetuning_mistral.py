...
Path(
    '/lustre/fsn1/projects/rech/knb/urc37ho/iclr/svd/38p/13M_SlimOrca/r1536/mistral-7b/teacher+student/bottom_layers/finetuned'
).mkdir(parents=True, exist_ok=True)
# Load model
_, model, config = load_lr_llm(checkpoint="/gpfsdswork/dataset/HuggingFace_Models/mistralai/Mistral-7B-v0.1/",
...
# Load data
train_data = load_from_disk("/lustre/fsn1/projects/rech/knb/urc37ho/SlimOrca-tokenized-mistral-2048")
...
def save_ckpt(model: torch.nn.Module,
              ...
              sum_loss: list,
              batch_idx: int):
    path = Path("/lustre/fsn1/projects/rech/knb/urc37ho/iclr/svd/38p/13M_SlimOrca/r1536/mistral-7b/teacher+student/bottom_layers/finetuned")
...
def train(model, tokenizer):
    ...
    model, optimizer, _, current_batch_idx, sum_loss = load_ckpt(model, optimizer, None)
    current_step = 0
    ...
    for batch_idx, batch in enumerate(tqdm(train_dataloader, desc="Training..."), start=1):
        ...
        if ((time.time() - start_time) / 60) > SAVE_EVEY_MIN:
            ...
            save_ckpt(model=model, optimizer=optimizer, lr_scheduler=None, sum_loss=sum_loss, batch_idx=batch_idx)
